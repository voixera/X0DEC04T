--═══════════════════════════════════════════════════════════════
-- X0DEC04T Hub v0.5.8 - Violence District
-- FIXED: Invisible (Void TP + Transparency hybrid)
-- FIXED: Auto Parry (3-layer system)
-- ADDED: Full keybind system for every feature
...
local INSTANCE_KEY = "__X0DEC04T_v058_INSTANCE"
...
local function Err(msg, d)
    warn(string.format("[X0DEC04T][+%.2fs] ERROR: %s | %s",
        os.clock()-_logStart, tostring(msg), tostring(d or "")))
end
...
local function FindRemotesByKeyword(root, keyword)
    ...
    for _, v in ipairs(root:GetDescendants()) do
        if (v:IsA("RemoteEvent") or v:IsA("RemoteFunction"))
           and v.Name:lower():find(keyword:lower()) then
...
do
    ...
    for _, r in ipairs(FindRemotesByKeyword(Remotes,"dodge")) do table.insert(R.Parry,r) end
    ...
    for _, r in ipairs(FindRemotesByKeyword(Remotes,"heal")) do table.insert(R.Heal,r) end
end
...
Log("Parry remotes: "..#R.Parry.." | Heal remotes: "..#R.Heal)
...
local State = {
    ...
    ParryCooldown=0.25, ParryOnHit=true,
    ...
    HPGui=nil, HPText=nil, HPBar=nil,
...
function Move.NearestGen()
    ...
    for _, g in ipairs(WS.Generators:GetChildren()) do
        local p = g:FindFirstChild("HitBox") or g.PrimaryPart
...
function HP.Create()
    ...
    hpText.Font = Enum.Font.GothamBold; hpText.TextSize = 14
...
-- ───────────────────────────────────────────
-- INVISIBLE — Void TP + Transparency hybrid
-- ───────────────────────────────────────────
...
local function PickVoidPos()
    -- Try to find a legit hidden spot first
    if WS.Map then
        for _, name in ipairs({"Hidden","SafeZone","Lobby","WaitingArea","Spawn"}) do
            local spot = WS.Map:FindFirstChild(name)
            if spot then
                local pt = spot:FindFirstChildWhichIsA("BasePart")
                if pt then return pt.Position + Vector3.new(0,3,0) end
            end
        end
    end
    return VOID_POSITIONS[1]
end
...
function Surv.SetInvisible(enable)
    ...
    if not enable then
        ...
        pcall(function() char:SetAttribute("Invisible",     false) end)
        ...
        if State.InvisibleSavedPos then
            ...
            if hrp then
                pcall(function()
                    hrp.CFrame = CFrame.new(State.InvisibleSavedPos)
                        + Vector3.new(0, 1, 0)
                end)
            end
        ...
        Notify("Invisible", "OFF — Back to map", 2)
        return
    end

    -- ── ON ───────────────────────────────────
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then Notify("Invisible","No HRP found",3); return end

    -- Save current position
    State.InvisibleSavedPos = hrp.Position

    -- Pick void/hidden position
    local voidPos = PickVoidPos()
    State.InvisibleVoidPos = voidPos

    -- Teleport to void
    pcall(function() hrp.CFrame = CFrame.new(voidPos) end)

    -- Make every part invisible
    ...
    -- Freeze humanoid so it doesn't walk back
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then
        pcall(function() hum.PlatformStand = true end)
    end

    -- Maintenance loop: keep void pos + keep invisible
    State.InvisibleConn = RunService.Heartbeat:Connect(function()
        if not State.Invisible then return end
        local c = LocalPlayer.Character; if not c then return end
        local h = c:FindFirstChild("HumanoidRootPart")

        -- Snap back if drifted
        if h and State.InvisibleVoidPos then
        ...
        -- Re-apply transparency every frame (games reset it)
        ...
        -- Keep server-side attributes
        pcall(function()
            c:SetAttribute("Untargettable", true)
            c:SetAttribute("Invisible",     true)
        end)
    end)

    Notify("Invisible", "ON — Teleported to void", 2)
end
...
local function DoParry(reason)
    ...
    if State.ParryDebug then
        Log("Parry fired | reason="..tostring(reason)
            .." t="..string.format("%.3f",now))
    end

    SetParryAttr(true)
    task.delay(0.25, function() SetParryAttr(false) end)

    if State.ParryMode ~= "Key Only"    then task.spawn(FireParryRemotes) end
    if State.ParryMode ~= "Remote Only" then task.spawn(FireParryKeys)    end
end
...
function Surv.SetAutoParry(enable)
    ...
    -- Layer 1: Heartbeat proximity + swing watcher
    ...
    -- Layer 3: Attribute polling loop
    task.spawn(function()
                ...
                if hrp and khrp then
                    ...
                    if dist <= State.ParryRange * 1.8 then
                        for _, attrName in ipairs({
                            "InAttack","Attacking","SwingActive",
                            "SwingCooldown","IsSwinging","hitlag","HitStop",
                        }) do
                            if kchar:GetAttribute(attrName) == true then
...
local function DoHeal()
    ...
    if State.AutoHealMethod == "Remote" or State.AutoHealMethod == "All" then
        ...
        for _, r in ipairs(R.Heal) do pcall(function() r:FireServer() end) end
    end
                ...
                if WS.Generators then
                    ...
                    if hrp then
                        if State.CurrentTargetGen then
                            local prog =
                                State.CurrentTargetGen:GetAttribute("RepairProgress") or 0
...
KB.Register("KillerESP", KB.Keys.KillerESP, function()
    ...
    Role.ResetKillerCache(); ESP.RefreshAll()
...
-- helper: safe keybind setter
local function MakeKBCallback(name)
    return function(k)
...
if Tabs.Main then
    ...
    T:CreateSection("Default Keybinds")
    T:CreateLabel("X=Invisible | P=AutoParry | Q=Force Parry")
    T:CreateLabel("H=AutoHeal  | Y=Force Heal | G=GodMode")
    T:CreateLabel("N=NoClip | J=InfJump | B=SpeedBoost | F=Flee")
    T:CreateLabel("L=FullBright | V=Freecam | R=AutoGen | K=Skillcheck")
    T:CreateLabel("O=GenProgress | U=HPBar | M=ServerHop | End=PanicESP")
    ...
    T:CreateButton({Name="Print All Remotes", Callback=function()
        if Remotes then
            Log("=== REMOTES ===")
    ...
    T:CreateButton({Name="Force Test Parry", Callback=function()
        State.LastParryTime = 0; DoParry("test")
...
if Tabs.Survivor then
    ...
    T:CreateToggle({Name="Invisible (Void TP + Transparent)", CurrentValue=false, Flag="Inv",
        Callback=function(v)
            State.Invisible = v
            Surv.SetInvisible(v)
        end})
    T:CreateButton({Name="Quick Toggle Invisible", Callback=function()
        State.Invisible = not State.Invisible
        Surv.SetInvisible(State.Invisible)
    end})
    ...
    T:CreateToggle({Name="Speed Boost", CurrentValue=false, Flag="SB",
        Callback=function(v)
            State.SurvSpeedBoost = v
            Surv.SetSpeedBoost(v)
        end})
    ...
    T:CreateSlider({Name="Parry Range (studs)", Range={5,80}, Increment=1, CurrentValue=20, Flag="PR",
        Callback=function(v)
    ...
    T:CreateDropdown({Name="Gen Mode",
        Options={"Instant","Legit"}, CurrentOption={"Instant"}, Flag="AGM",
...
if Tabs.ESP then
    ...
    T:CreateSlider({Name="Max Distance (studs)", Range={50,2000}, Increment=50,
        CurrentValue=500, Flag="E11",
...
if Tabs.Keybinds then
    ...
    T:CreateKeybind({Name="Auto Parry Toggle", CurrentKeybind="P",
        HoldToInteract=false, Flag="KB_AP",
        Callback=MakeKBCallback("AutoParry")})
    T:CreateKeybind({Name="Force Parry (Manual)", CurrentKeybind="Q",
        HoldToInteract=false, Flag="KB_FP",
        Callback=MakeKBCallback("ForceParry")})
    T:CreateKeybind({Name="Auto Heal Toggle", CurrentKeybind="H",
        HoldToInteract=false, Flag="KB_AH",
        Callback=MakeKBCallback("AutoHeal")})
    T:CreateKeybind({Name="Force Heal (Manual)", CurrentKeybind="Y",
        HoldToInteract=false, Flag="KB_FH",
        Callback=MakeKBCallback("ForceHeal")})
    T:CreateKeybind({Name="God Mode Toggle", CurrentKeybind="G",
        HoldToInteract=false, Flag="KB_GM",
        Callback=MakeKBCallback("GodMode")})
    T:CreateKeybind({Name="Flee Killer Toggle", CurrentKeybind="F",
        HoldToInteract=false, Flag="KB_FL",
        Callback=MakeKBCallback("FleeKiller")})

    T:CreateSection("Character")
    T:CreateKeybind({Name="Invisible Toggle", CurrentKeybind="X",
        HoldToInteract=false, Flag="KB_INV",
        Callback=MakeKBCallback("Invisible")})
    T:CreateKeybind({Name="Speed Boost Toggle", CurrentKeybind="B",
        HoldToInteract=false, Flag="KB_SB",
        Callback=MakeKBCallback("SpeedBoost")})

    T:CreateSection("Movement")
    T:CreateKeybind({Name="NoClip Toggle", CurrentKeybind="N",
        HoldToInteract=false, Flag="KB_NC",
        Callback=MakeKBCallback("NoClip")})
    T:CreateKeybind({Name="Infinite Jump Toggle", CurrentKeybind="J",
        HoldToInteract=false, Flag="KB_IJ",
        Callback=MakeKBCallback("InfJump")})

    T:CreateSection("Generators")
    T:CreateKeybind({Name="Auto Gen Rush Toggle", CurrentKeybind="R",
        HoldToInteract=false, Flag="KB_AGR",
        Callback=MakeKBCallback("AutoGenRush")})
    T:CreateKeybind({Name="Auto Skillcheck Toggle", CurrentKeybind="K",
        HoldToInteract=false, Flag="KB_ASC",
        Callback=MakeKBCallback("AutoSkillcheck")})
    T:CreateKeybind({Name="Gen Progress Toggle", CurrentKeybind="O",
        HoldToInteract=false, Flag="KB_GP",
        Callback=MakeKBCallback("GenProgress")})

    T:CreateSection("Visuals")
    T:CreateKeybind({Name="FullBright Toggle", CurrentKeybind="L",
        HoldToInteract=false, Flag="KB_FB",
        Callback=MakeKBCallback("FullBright")})
    T:CreateKeybind({Name="Freecam Toggle", CurrentKeybind="V",
        HoldToInteract=false, Flag="KB_FC",
        Callback=MakeKBCallback("Freecam")})
    T:CreateKeybind({Name="HP Bar Toggle", CurrentKeybind="U",
        HoldToInteract=false, Flag="KB_HP",
        Callback=MakeKBCallback("OwnHP")})
    T:CreateKeybind({Name="No Fog Toggle", CurrentKeybind="F7",
        HoldToInteract=false, Flag="KB_NF",
        Callback=MakeKBCallback("NoFog")})
    T:CreateKeybind({Name="No Shadows Toggle", CurrentKeybind="F8",
        HoldToInteract=false, Flag="KB_NS",
        Callback=MakeKBCallback("NoShadows")})

    T:CreateSection("ESP")
    T:CreateKeybind({Name="Killer ESP Toggle", CurrentKeybind="F1",
        HoldToInteract=false, Flag="KB_KE",
        Callback=MakeKBCallback("KillerESP")})
    T:CreateKeybind({Name="Survivor ESP Toggle", CurrentKeybind="F2",
        HoldToInteract=false, Flag="KB_SE",
        Callback=MakeKBCallback("SurvivorESP")})
    T:CreateKeybind({Name="Generator ESP Toggle", CurrentKeybind="F3",
        HoldToInteract=false, Flag="KB_GE",
        Callback=MakeKBCallback("GeneratorESP")})
    T:CreateKeybind({Name="Pallet ESP Toggle", CurrentKeybind="F4",
        HoldToInteract=false, Flag="KB_PE",
        Callback=MakeKBCallback("PalletESP")})
    T:CreateKeybind({Name="Vault ESP Toggle", CurrentKeybind="F5",
        HoldToInteract=false, Flag="KB_VE",
        Callback=MakeKBCallback("VaultESP")})
    T:CreateKeybind({Name="Hook ESP Toggle", CurrentKeybind="F6",
        HoldToInteract=false, Flag="KB_HE",
        Callback=MakeKBCallback("HookESP")})

    T:CreateSection("Misc")
    T:CreateKeybind({Name="Server Hop", CurrentKeybind="M",
        HoldToInteract=false, Flag="KB_SH",
        Callback=MakeKBCallback("ServerHop")})
    T:CreateKeybind({Name="Panic Clear ESP", CurrentKeybind="End",
        HoldToInteract=false, Flag="KB_PAN",
        Callback=MakeKBCallback("PanicClearESP")})
end
...
CM:Add(LocalPlayer.CharacterAdded, function()
    ...
    State.InvisibleSavedPos    = nil
    State.InvisibleVoidPos     = nil
    -- If invisible was on, turn it off cleanly first
...
_G[INSTANCE_KEY] = {
    ...
    destroy   = function()
        ...
        State.AutoParry=false; State.Invisible=false
        Surv.SetGenProgress(false); Surv.SetInvisible(false)
        Surv.SetAutoSkillcheck(false); Surv.SetAutoHeal(false)
        Surv.SetAutoParry(false)
        if State.ParryRing then pcall(function() State.ParryRing:Destroy() end) end
...
Notify(HUB.Name, "v"..HUB.Version.." ready! | "..#R.Parry.." parry remotes", 5)
Log("Ready — v0.5.8 fully loaded")
