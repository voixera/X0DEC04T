--═══════════════════════════════════════════════════════════════
-- X0DEC04T Hub v2.5.0 - Border RP (Scarface)
--═══════════════════════════════════════════════════════════════
...
-- DUPLICATE GUARD
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
local INSTANCE_KEY = "__X0DEC04T_BRP_v250"
...
local function Err(m,d) warn(string.format("[X0DEC04T][+%.2fs] ERR: %s | %s", os.clock()-_t0, tostring(m), tostring(d or ""))) end

Log("BorderRP Hub v2.5.0 starting...")
...
local HUB = { Name="X0DEC04T Hub", Game="Border RP", Version="2.5.0", Author="voixera" }
...
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- BORDER RP REMOTES (Confirmed paths)
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
local RemotesFolder
...
local BRP = {
    ...
    Release                 = GetRemote("Handcuffs.Release"),
    BatonSwing              = GetRemote("Baton.Swing"),
    SearchPlayer            = GetRemote("Tablet.SearchPlayer"),
    SearchWarrant           = GetRemote("Tablet.SearchWarrantTarget"),
...
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- CONFIRMED WORKSPACE PATHS (from your dump)
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
local BRP_PATHS = {
    ...
    ElCapoSpawn  = Workspace:FindFirstChild("ElCapoSpawnTeleport"),
}
Log("NPC folder: " .. (BRP_PATHS.NPC and "OK" or "MISSING"))
Log("Vehicles folder: " .. (BRP_PATHS.Vehicles and "OK" or "MISSING"))
Log("LaunderTrigger: " .. (BRP_PATHS.LaunderTrigger and "OK" or "MISSING"))
Log("ElCapoSpawn: " .. (BRP_PATHS.ElCapoSpawn and "OK" or "MISSING"))

--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- CONFIRMED POSITIONS (from your screenshots)
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
local DEFAULT_POS = {
    SellNPC = Vector3.new(209.93, 17.23, -46.54),      -- Seller4
    Laundry = Vector3.new(6804.78, 17.43, -34.70),     -- LaunderTrigger
    Shop    = Vector3.new(6823.57, 17.40, -20.00),     -- El Capo Buyable Items
    ElCapo  = Vector3.new(6823.57, 17.40, -20.00),     -- El Capo teleport pad
}
...
local State = {
    -- ESP
    ESP_Players=false, ESP_Cars=false, ESP_Wanted=false, ESP_Police=false,
    ...
    -- SMUGGLER (using confirmed positions as defaults)
    Smuggler_AutoLoop     = false,
    Smuggler_Status       = "Idle",
    Smuggler_JobsDone     = 0,
    Smuggler_Earnings     = 0,
    Smuggler_StartTime    = 0,
    Smuggler_Delay        = 2,
    Smuggler_StatusLbl    = nil,
    Smuggler_UseRemotes   = true,
    Smuggler_AutoLaunder  = true,
    Smuggler_UseSpawnedCar = false,   -- Skip vehicle spawn (TP directly)
    ...
    Smuggler_SellerName = "Seller4",  -- default; you can pick Seller1..N
    Smuggler_BuyKeyword = "buy",       -- keyword to match on buy prompt
...
local function FormatTime(s)
    ...
    if h > 0 then return string.format("%02d:%02d:%02d", h, m, ss)
    else return string.format("%02d:%02d", m, ss) end
end
...
local function GetMoney()
    ...
    if ls then
        for _, v in ipairs(ls:GetChildren()) do
            ...
            if n:find("money") or n:find("cash") or n:find("balance") or n:find("dollar") then
...
function ESP.Clear(o)
    ...
    for _, inst in pairs(c) do if typeof(inst)=="Instance" and inst.Parent then pcall(function() inst:Destroy() end) end end
    State.ESPCache[o] = nil
end
function ESP.ClearAll() for o in pairs(State.ESPCache) do ESP.Clear(o) end; State.ESPCache = {} end
local function MakeBB(a, label, color, sn, sd, md)
    local bb = Instance.new("BillboardGui")
    bb.Adornee=a; bb.Size=UDim2.new(0,260,0,55); bb.StudsOffset=Vector3.new(0,4,0)
    bb.AlwaysOnTop=true; bb.LightInfluence=0; bb.MaxDistance=md; bb.Parent=GuiParent()
    local nl = Instance.new("TextLabel", bb)
    nl.Size=UDim2.new(1,0,0.6,0); nl.BackgroundTransparency=1; nl.Text=tostring(label)
    nl.TextColor3=color; nl.TextStrokeTransparency=0; nl.Font=Enum.Font.GothamBold; nl.TextSize=14; nl.Visible=sn
    local dl = Instance.new("TextLabel", bb)
    dl.Size=UDim2.new(1,0,0.4,0); dl.Position=UDim2.new(0,0,0.6,0); dl.BackgroundTransparency=1; dl.Text="0m"
    dl.TextColor3=Color3.fromRGB(220,220,220); dl.TextStrokeTransparency=0; dl.Font=Enum.Font.Gotham; dl.TextSize=12; dl.Visible=sd
    return bb, nl, dl
end
function ESP.AddTarget(a, model, label, color)
    ...
    local bb, nl, dl = MakeBB(a, label, color, State.ESP_ShowName, State.ESP_ShowDist, State.ESP_MaxDist)
...
function Car.SetSpeedHack(e)
    ...
    if e then State.SpeedHackConn = RunService.Heartbeat:Connect(function()
        local seat = GetVehicleSeat(); if seat then seat.MaxSpeed = tonumber(State.SpeedHackValue) or 500 end
    end) end
end
...
function Car.SavePos(label)
    ...
    table.insert(State.SavedPositions, {name=label, cframe=h.CFrame})
    Notify("Saved", label, 2)
end
function Car.LoadPos(label)
    for _, e in ipairs(State.SavedPositions) do
        if e.name == label then
            local r = GetCarRoot()
            if r then r.CFrame = e.cframe + Vector3.new(0,3,0)
            else local h = GetHRP(); if h then h.CFrame = e.cframe end end
            return
        end
    end
end
...
-- Get the specific seller prompt (Seller1..N) by name
function Smuggler.GetSellPrompt(sellerName)
    ...
    if not seller then
        -- Fallback: find any Seller* with SellSmuggledGoodsPrompt
        for _, npc in ipairs(BRP_PATHS.NPC:GetChildren()) do
            if npc.Name:lower():find("seller") then
                local hrp = npc:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local p = hrp:FindFirstChild("SellSmuggledGoodsPrompt")
                    if p then return p, npc end
                end
            end
        end
        return nil
...
-- Find nearest buy prompt near shop
function Smuggler.GetBuyPromptNear(pos, radius)
    if not pos then return nil end
    radius = radius or 15
    local kw = (State.Smuggler_BuyKeyword or "buy"):lower()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and obj.Enabled then
            ...
            if part and part:IsA("BasePart") then
                ...
                if d <= radius then
                    local a = obj.ActionText:lower(); local o = obj.ObjectText:lower()
                    -- Match Indonesian: "beli" or English: "buy" or "purchase"
                    if a:find(kw, 1, true) or a:find("beli") or a:find("buy") or a:find("purchase") 
                    or o:find(kw, 1, true) or o:find("beli") then
...
function Smuggler.BuyItem()
    Smuggler.SetStatus("Teleporting to shop")
    Smuggler.TP(State.POS_Shop, 3)
    task.wait(0.6)
    
    Smuggler.SetStatus("Buying item")
    local prompt = Smuggler.GetBuyPromptNear(State.POS_Shop, 20)
    if prompt then
        Smuggler.FirePrompt(prompt)
        Log("Fired buy prompt: " .. prompt:GetFullName())
        task.wait(0.5)
    else
        -- Fallback: fire any prompt within 10 studs
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                local part = obj.Parent
                if part and part:IsA("BasePart") then
                    local d = (part.Position - State.POS_Shop).Magnitude
                    if d <= 8 then
                        Smuggler.FirePrompt(obj)
                        Log("Fallback fired: " .. obj:GetFullName())
                        break
                    end
                end
            end
...
-- Step 2: Go to sell NPC and sell
function Smuggler.SellItems()
    Smuggler.SetStatus("Teleporting to seller")
    
    -- Try to find the seller NPC by name for exact positioning
    local prompt, seller = Smuggler.GetSellPrompt(State.Smuggler_SellerName)
    if seller then
        local hrp = seller:FindFirstChild("HumanoidRootPart")
        if hrp then
            -- TP right next to seller
            local myHRP = GetHRP()
            if myHRP then
                myHRP.CFrame = hrp.CFrame + hrp.CFrame.LookVector * -3 + Vector3.new(0, 0, 0)
                task.wait(0.5)
            end
        end
    else
    ...
    Smuggler.SetStatus("Selling goods")
    local moneyBefore = GetMoney()
    
    -- Fire the specific SellSmuggledGoodsPrompt
    if prompt then
        Smuggler.FirePrompt(prompt)
        Log("Fired sell prompt: " .. prompt:GetFullName())
        task.wait(0.8)
    end
    
    -- Also try remote as backup
    ...
    Smuggler.SetStatus("Sold #" .. State.Smuggler_JobsDone)
    return true
end
...
function Smuggler.LaunderMoney()
    ...
    Smuggler.SetStatus("Teleporting to laundry")
    Smuggler.TP(State.POS_Laundry, 3)
    task.wait(0.5)
    
    Smuggler.SetStatus("Laundering cash")
    local prompt = Smuggler.GetLaunderPrompt()
    if prompt then
        Smuggler.FirePrompt(prompt)
        Log("Fired launder prompt: " .. prompt:GetFullName())
        task.wait(0.5)
    end
...
function Smuggler.SetAutoLoop(e)
    ...
    if not e then
        Smuggler.SetStatus("Loop stopped")
        Overlay.Hide()
        return
    end
    State.Smuggler_StartTime = tick()
    State.Smuggler_Earnings = 0
    State.Smuggler_JobsDone = 0
    Smuggler.SetStatus("Loop started")
...
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- POLICE (Aimbot + Auto Arrest with Handcuffs.Detain)
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
local Police = {}
function Police.CreateFOV()
    ...
    if Drawing then
        ...
        State.Police_FOVCircle.Thickness = 1
        State.Police_FOVCircle.NumSides = 60
        State.Police_FOVCircle.Radius = State.Police_AimFOV
        State.Police_FOVCircle.Filled = false
        State.Police_FOVCircle.Visible = false
        State.Police_FOVCircle.Transparency = 0.7
...
function Police.AimAt(part)
    ...
    if State.Police_AimSmooth <= 1 then
        Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, tp)
    else
        local cur = Camera.CFrame
        Camera.CFrame = cur:Lerp(CFrame.lookAt(cur.Position, tp), math.clamp(1/State.Police_AimSmooth, 0.05, 1))
    end
end
...
function Police.TryArrest()
    ...
    if BRP.Jail then
        task.wait(0.4)
        pcall(function() BRP.Jail:InvokeServer(t) end)
    end
end
...
function Police.SetAutoArrest(e)
    ...
    if e then
        ...
        State.Police_ArrestConn = RunService.Heartbeat:Connect(function()
            if State.Police_CurrentTarget and tick() - last >= 2.5 then
                last = tick(); Police.TryArrest()
            end
        end)
    end
end
...
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-- Get available seller names for dropdown
--━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = HUB.Game .. " - by " .. HUB.Author,
    Theme = "Default",
...
if Tabs.Main then
    ...
    Sec(T, "Wired Systems")
    Lbl(T, "SellSmuggledGoods: " .. (BRP.SellSmuggledGoods and "✅" or "❌"))
    Lbl(T, "LaunderBriefcase:  " .. (BRP.LaunderBriefcase and "✅" or "❌"))
    Lbl(T, "SpawnVehicle:      " .. (BRP.SpawnVehicleFromSpawner and "✅" or "❌"))
    Lbl(T, "Handcuffs.Detain:  " .. (BRP.Detain and "✅" or "❌"))
    Lbl(T, "Baton.Swing:       " .. (BRP.BatonSwing and "✅" or "❌"))
    Lbl(T, "Total remotes:     " .. #RemoteDBAll)
    
    Sec(T, "Confirmed Positions")
    Lbl(T, "Shop:    " .. tostring(DEFAULT_POS.Shop))
    Lbl(T, "SellNPC: " .. tostring(DEFAULT_POS.SellNPC))
    Lbl(T, "Laundry: " .. tostring(DEFAULT_POS.Laundry))
    
    Sec(T, "Player")
    Lbl(T, "Team: " .. (LocalPlayer.Team and LocalPlayer.Team.Name or "None"))
    Lbl(T, "Rank: " .. GetRank(LocalPlayer))
    Lbl(T, "Wanted Level: " .. GetWantedLevel(LocalPlayer))
end
...
if Tabs.Car then
    ...
    Tog(T, {Name="Speed Hack (Constant)", CurrentValue=false, Flag="SH",
    ...
    Btn(T, {Name="Unstuck (Remote)", Callback=function()
        if BRP.UnstuckVehicle then pcall(function() BRP.UnstuckVehicle:FireServer() end) end
    end})
    
    Sec(T, "Abilities")
    Tog(T, {Name="NoClip", CurrentValue=false, Flag="NC",
        Callback=function(v) State.NoClip=v; Car.SetNoClip(v) end})
    Tog(T, {Name="Fly (on foot)", CurrentValue=false, Flag="FL",
        Callback=function(v) State.FlyActive=v; Car.SetFly(v) end})
    Tog(T, {Name="God Mode", CurrentValue=false, Flag="GM",
        Callback=function(v) State.GodMode=v; Car.SetGod(v) end})
    ...
    Tog(T, {Name="Infinite Jump", CurrentValue=false, Flag="IJ",
...
if Tabs.Smuggler then
    ...
    Sec(T, "🚀 Auto Smuggler - Border RP")
    Lbl(T, "Loop: Buy → Sell → Launder")
    Lbl(T, "Uses real prompts + remotes")
    ...
    Drp(T, {Name="Target Seller NPC", Options=sellerNames,
    ...
    Tog(T, {Name="Use Remotes (recommended)", CurrentValue=true, Flag="UR",
    ...
    Sec(T, "Manual Steps")
    Btn(T, {Name="1️⃣ Buy Item (TP to shop)", Callback=function() task.spawn(Smuggler.BuyItem) end})
    Btn(T, {Name="2️⃣ Sell to NPC (TP + sell)", Callback=function() task.spawn(Smuggler.SellItems) end})
    Btn(T, {Name="3️⃣ Launder Money (TP + launder)", Callback=function() task.spawn(Smuggler.LaunderMoney) end})
    
    Sec(T, "Direct Fire (Remotes)")
    Btn(T, {Name="Fire SellSmuggledGoods", Callback=function()
        if BRP.SellSmuggledGoods then
            pcall(function() BRP.SellSmuggledGoods:FireServer() end)
            Notify("Remote", "Sell fired", 2)
        end
    end})
    Btn(T, {Name="Fire LaunderBriefcase", Callback=function()
        if BRP.LaunderBriefcase then
            pcall(function() BRP.LaunderBriefcase:FireServer() end)
            Notify("Remote", "Launder fired", 2)
        end
    end})
    
    Sec(T, "Custom Positions (Override)")
    Btn(T, {Name="Override Shop Position (Here)", Callback=function()
        local h = GetHRP(); if h then State.POS_Shop = h.Position; Notify("Shop", "Updated", 2) end
    end})
    Btn(T, {Name="Override Sell Position (Here)", Callback=function()
        local h = GetHRP(); if h then State.POS_Sell = h.Position; Notify("Sell", "Updated", 2) end
    end})
    Btn(T, {Name="Override Laundry Position (Here)", Callback=function()
        local h = GetHRP(); if h then State.POS_Laundry = h.Position; Notify("Laundry", "Updated", 2) end
    end})
    Btn(T, {Name="Reset to Defaults", Callback=function()
        State.POS_Shop = DEFAULT_POS.Shop
        State.POS_Sell = DEFAULT_POS.SellNPC
        State.POS_Laundry = DEFAULT_POS.Laundry
        Notify("Reset", "Using confirmed positions", 3)
    end})
    
    Sec(T, "Debug")
    Btn(T, {Name="List All Sellers", Callback=function()
        if not BRP_PATHS.NPC then Notify("Error", "No NPC folder", 3); return end
        for _, npc in ipairs(BRP_PATHS.NPC:GetChildren()) do
            if npc.Name:lower():find("seller") then
                local hrp = npc:FindFirstChild("HumanoidRootPart")
                local prompt = hrp and hrp:FindFirstChild("SellSmuggledGoodsPrompt")
                Log(string.format("  %s | HRP:%s | Prompt:%s | Pos:%s",
                    npc.Name, tostring(hrp and true), tostring(prompt and true),
                    hrp and tostring(hrp.Position) or "N/A"))
            end
        end
    end})
    Btn(T, {Name="Find Buy Prompt Near Me", Callback=function()
        local h = GetHRP(); if not h then return end
        local p = Smuggler.GetBuyPromptNear(h.Position, 20)
        if p then
            Notify("Buy Prompt", "Found: " .. p.ActionText, 4)
            Log("Buy prompt: " .. p:GetFullName())
        else
            Notify("Buy Prompt", "Not found - move closer to item", 3)
        end
    end})
...
if Tabs.Police then
    ...
    Sec(T, "🔫 Police Aimbot")
    Lbl(T, "Auto-target wanted smugglers")
    Lbl(T, "Uses WantedLevel attribute")
    
    Sec(T, "Targeting")
    Tog(T, {Name="Target Wanted Only", CurrentValue=true, Flag="TW",
        Callback=function(v) State.Police_TargetWanted=v end})
    Tog(T, {Name="Target ALL Players", CurrentValue=false, Flag="TA",
        Callback=function(v) State.Police_TargetAll=v end})
    ...
    Tog(T, {Name="Show FOV Circle", CurrentValue=true, Flag="SFOV",
        Callback=function(v) State.Police_ShowFOV=v end})
    
    Sec(T, "⚡ Controls")
    Tog(T, {Name="🎯 Auto-Aim (RMB snap)", CurrentValue=false, Flag="AAim",
        Callback=function(v) Police.SetAutoAim(v) end})
    Tog(T, {Name="🔥 Auto-Fire", CurrentValue=false, Flag="AFire",
        Callback=function(v) Police.SetAutoFire(v) end})
    Tog(T, {Name="🚔 Auto-Arrest (TP + Detain)", CurrentValue=false, Flag="AArr",
        Callback=function(v) Police.SetAutoArrest(v) end})
    ...
    Btn(T, {Name="TP to Nearest Wanted", Callback=function()
        ...
        if best and best.Character then
            ...
            if hrp then
                myHRP.CFrame = hrp.CFrame + hrp.CFrame.LookVector * -5 + Vector3.new(0, 1, 0)
                Notify("TP", "-> " .. best.Name .. " (★" .. GetWantedLevel(best) .. ")", 3)
            end
        else Notify("TP", "No wanted", 3) end
    end})
    Btn(T, {Name="List Wanted Players", Callback=function()
        local c = 0
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and IsWanted(p) then
                c = c + 1
                Log(string.format("  %s | ★%d | Rank: %s", p.Name, GetWantedLevel(p), GetRank(p)))
            end
        end
        Notify("Wanted", c .. " players (console)", 3)
    end})
    Btn(T, {Name="Detain Current Target", Callback=function()
        if State.Police_CurrentTarget and BRP.Detain then
            ...
            Notify("Detain", "Fired on " .. State.Police_CurrentTarget.Name, 3)
        end
    end})
    Btn(T, {Name="Jail Current Target", Callback=function()
        if State.Police_CurrentTarget and BRP.Jail then
            pcall(function() BRP.Jail:InvokeServer(State.Police_CurrentTarget) end)
        end
    end})
end
...
if Tabs.Teleport then
    ...
    Sec(T, "Player TP")
    Inp(T, {Name="Player Name", PlaceholderText="Case-sensitive", RemoveTextAfterFocusLost=false,
    ...
    Drp(T, {Name="Select Player", Options=#names>0 and names or {"None"}, CurrentOption={names[1] or "None"},
        MultiOption=false, Flag="TPD",
        Callback=function(v) Car.TeleportToPlayer((type(v)=="table" and v[1]) or v) end})
    
    Sec(T, "Border RP Quick TP")
    Btn(T, {Name="🏪 TP to Shop (El Capo)", Callback=function() Smuggler.TP(State.POS_Shop, 3) end})
    Btn(T, {Name="💰 TP to Sell NPC (Seller4)", Callback=function() Smuggler.TP(State.POS_Sell, 3) end})
    Btn(T, {Name="🧺 TP to Laundry", Callback=function() Smuggler.TP(State.POS_Laundry, 3) end})
    
    Sec(T, "Saved Positions")
    local sp = ""
    Inp(T, {Name="Label", PlaceholderText="e.g. MyBase", RemoveTextAfterFocusLost=false,
        Callback=function(v) sp=v end})
    Btn(T, {Name="Save Current", Callback=function() Car.SavePos(sp) end})
    Btn(T, {Name="Load", Callback=function() Car.LoadPos(sp) end})
end
...
if Tabs.ESP then
    local T = Tabs.ESP
    Sec(T, "ESP")
    Tog(T, {Name="All Players ESP", CurrentValue=false, Flag="PE",
        Callback=function(v)
            State.ESP_Players = v
            if not v then
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character then
                        ...
                        if h and not (State.ESP_Wanted and IsWanted(p)) then ESP.Clear(h) end
                    end
                end
            end
        end})
    Tog(T, {Name="Wanted ESP (Red)", CurrentValue=false, Flag="WE",
        Callback=function(v) State.ESP_Wanted=v end})
    Tog(T, {Name="Police ESP (Blue)", CurrentValue=false, Flag="POE",
        Callback=function(v) State.ESP_Police=v end})
...
if Tabs.Remote then
    ...
    Inp(T, {Name="Name", PlaceholderText="e.g. SellSmuggledGoods", RemoveTextAfterFocusLost=false,
    ...
    Btn(T, {Name="Fire", Callback=function()
        ...
        pcall(function()
            if r:IsA("RemoteEvent") then r:FireServer(a) else r:InvokeServer(a) end
        end)
...
if Tabs.Visuals then
    ...
    Sec(T, "Lighting")
    Tog(T, {Name="FullBright", CurrentValue=false, Flag="FB",
        Callback=function(v) State.FullBright=v; Vis.FullBright(v) end})
    Tog(T, {Name="No Fog", CurrentValue=false, Flag="NF",
        Callback=function(v) State.NoFog=v; Vis.NoFog(v) end})
    Sld(T, {Name="Time", Range={0,24}, Increment=1, CurrentValue=14, Flag="TOD",
        Callback=function(v) Vis.SetClock(v) end})
    Sec(T, "Camera")
    Sld(T, {Name="FOV", Range={30,120}, Increment=5, CurrentValue=70, Flag="FOV2",
        Callback=function(v) State.FOV=v; Vis.SetFOV(v) end})
end
...
if Tabs.Misc then
    ...
    Btn(T, {Name="Rejoin", Callback=function()
        pcall(function() game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer) end)
    end})
    Sec(T, "Position")
    Btn(T, {Name="Print & Copy Current Pos", Callback=function()
        local h = GetHRP()
        if h then
            local p = h.Position
...
if Tabs.Settings then
    ...
    Btn(T, {Name="🗑 Unload Hub", Callback=function()
        ...
        CM:Cleanup()
        Vis.RestoreLight(); Vis.MuteAll(false)
...
CM:Add(LocalPlayer.CharacterAdded, function()
    ...
    if State.JumpPower ~= 50 then local h=GetHuman(); if h then h.JumpPower=State.JumpPower end end
end)
...
Notify(HUB.Name, "v" .. HUB.Version .. " - Border RP Ready!", 5)
Log("Init complete | " .. remoteCount .. " remotes wired")
